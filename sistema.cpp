...
/**@brief Função para inicializar um tratador*/
...
/**@brief Função para inicializar um veterinario*/
int Sistema::funcaoVeterinario(){
	Veterinario* novoV = new Veterinario();
	int op;
	double aux;
	string aux1;
	int aux2;
...
void Sistema::registrarAnimal(){
	...
	Mamifero* novoMamifero = dynamic_cast<Mamifero*>(novo);
	Ave* novaAve = dynamic_cast<Ave*>(novo);
	Anfibio* novoAnfibio = dynamic_cast<Anfibio*>(novo);
	Reptil* novoReptil = dynamic_cast<Reptil*>(novo);
	switch(op){
		case 1: 
		...
		case 2: 
			...
			mapAve.insert(pair<int, Ave*>(novaAve->getId(), novaAve));
			break;
		case 3: 
